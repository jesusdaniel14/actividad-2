...
void llenar(int cad, Libro vt[]) {
	for (int i = 0; i < cad; i++) {
		vt[i].snbi = i + 1;
		vt[i].tituloLibro = "Libro" + to_string(i + 1);
		vt[i].autorLibro = "Autor" + to_string(i + 1);
...
void buscar_repetidos(const Usuario usuario[], int cad, const string& nombre, int index) {
	if (index < cad) {
		if (usuario[index].nombre == nombre) {
			cout << "Usuario repetido encontrado:" << endl;
			imprimir(usuario[index], cad, index);
		}
		buscar_repetidos(usuario, cad, nombre, index + 1);
	}
}
...
int menu(Libro vt[], int cad, Usuario& usuario, Empleado& empleado, int longitudNombre, int longitudApellido, const string& titulo, const string& nombre, int index) {
	while (true) {
		cout << "\nMENU" << endl;
		...
		case 2:
			while (true) {
				...
				case 1:
					...
					break;
				case 2:
					imprimir(usuario, cad, 0);
					break;
				case 3:
					imprimir(empleado, cad, 0);
					break;
				case 4:
					return 0;
				default:
		...
		case 3:
			while (true) {
				...
				case 2:
				{
					...
					cin >> nombre_a_buscar;
					int indice_usuario = buscar(usuario, cad, nombre_a_buscar,0);
					if (indice_usuario != -1) {
...
int main() {
	...
	// Llamada al menú
	menu(vt, CAD, usuario, empleado, 6, 6, "", "", 0);
